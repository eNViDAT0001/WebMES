...
const ProductBrand = () => {
    ...
    return(
        <div>
            ...
            <div>
                {CATEGORY_DUMMY_DATABASE.branch.map(data =>(
                    <div key={data.id} className="flex flex-row items-center mb-2">
                    <input type="checkbox" className=" w-4 h-4 mr-9px accent-purple-check-product-brand"/> 
                    <h1 className=" font-normal text-base text-gray-text-product-brand ">{data.name}</h1>
                </div>))}
            </div>
        </div>)
}
